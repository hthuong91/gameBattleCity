...
void Renderer::clear()
{
    ...
    SDL_RenderClear(m_renderer); //xóa bộ đệm phía sau (back buffer)

}

void Renderer::flush()
{
    SDL_RenderPresent(m_renderer); //hoán đổi các bộ đệm (buffer)

}

void Renderer::drawObject(const SDL_Rect *texture_src, const SDL_Rect *window_dest)
{
    SDL_RenderCopy(m_renderer, m_texture, texture_src, window_dest); //chúng ta đang vẽ lên bộ đệm phía sau (back buffer)
